 ...
 * - Deployed modules (such as OOB modules and deployed ModuleBuilder modules) that are located in the /modules directory and have metadata in modules/<name>/metadat                                                                        a and in the custom directory
...
abstract class AbstractMetaDataImplementation
{
     ...
     * Obtains the additional variables, such as module_name, to include in beginning of the file (as required by ModuleBuilder) from the internal variable _variable                                                                        s, set in the Constructor
     * @param string $filename       The full path to the file to contain the layout
     * @param array $defs               Array containing the layout definition; the top level should be the definition itself; not the modulename or viewdef= preambl                                                                        es found in the file definitions
        ...
        foreach ($layout as $key => $def) {
            ...
            if (is_array($def)) {
                if (isset ($def ['name']) && !is_array($def ['name'])) // found a 'name' definition, that is not the definition of a field called name :)
                {
                    ...
                    if (isset ($def ['label']) || isset ($def ['vname']) || isset($def ['widget_class'])) // dealing with a listlayout which lacks 'name' keys, but w                                                                        hich does have 'label' keys
                    {
                        $key = strtolower($key);
